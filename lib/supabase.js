...
export async function validateInviteCode(supabase, code) {
  ...
  if (!data.active) return { valid: false, reason: 'Invite code inactive' };
  if (data.expires_at && new Date(data.expires_at) < new Date()) return { valid: false, reason: 'Invite code expired' };
  if (data.used_count >= data.max_uses) return { valid: false, reason: 'Invite code fully used' };
  return { valid: true, code: data };
}

export async function markInviteCodeUsed(supabase, code) {
  const { error } = await supabase
    .rpc('consume_invite_code', { p_code: code.trim().toUpperCase() });
  if (error) throw error;
}
